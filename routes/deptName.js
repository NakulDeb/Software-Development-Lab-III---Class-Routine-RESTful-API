...
// Update by Department Name & Year.
// http://localhost:4000/deptName/dept/eee/2nd
router.put("/dept/:departments/:years",function(req,res,next){
    Dept.updateMany({department: (req.params.departments).toUpperCase(), year: (req.params.years)},req.body).then(function(dept){
        Dept.find({department: (req.params.departments).toUpperCase(), year: (req.params.years)}).then(function(dept){
            res.send(dept);
...
// Delete by Department Name & Year.
// http://localhost:4000/deptName/dept/eee/2nd
router.delete("/dept/:departments/:years",function(req,res,next){
    Dept.remove({department: (req.params.departments).toUpperCase(), year: (req.params.years).toLowerCase()}).then(function(dept){
        res.send(dept);
        if(dept){
            console.log("Delete request successful.");
        }
